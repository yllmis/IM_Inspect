...
Connector 调用还会接收由服务端 `ToolContext` 生成的请求上下文：`tenantId`、`actorId`、`requestId` 和 `runId`。这些字段不属于模型工具参数，不能由客服文本、模型输出或请求体覆盖；真实 Connector 应使用 `tenantId` 执行租户隔离，并使用 `actorId` 记录对象访问审计。
...
- 工具输入 Schema 使用 `.strict()` 拒绝 `sql`、`shell`、`query` 等未声明字段；即使字符串内容看起来像 SQL，也不会被当作可执行查询传递给 Connector。
